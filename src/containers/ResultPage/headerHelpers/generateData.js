export default (data, columnsArray) => {
  const flatColumns = columnsArray.reduce(
    (acc, column) => [...acc, ...column.columns],
    []
  );
  ...
  const createRow = (columns, row, rowIndex) => {
    ...
    columns.map((column) => {
      ...
      column.cellIndexes.map((cellIndex) => {
        ...
        switch (cellIndex[1]) {
          case "price($)":
            updatedRow["Цена"]["price($)"] = calculator(
              updatedRow["Цена"]["price($)"],
              row[cellIndex[0]]
            );
            break;
          case "dMin":
            updatedRow["Дней"]["dMin"] = calculator(
              updatedRow["Дней"]["dMin"],
              row[cellIndex[0]]
            );
            break;
          case "dMax(connect(…))":
            updatedRow["Дней"]["dMax(connect(…))"] = calculator(
              updatedRow["Дней"]["dMax(connect(…))"],
              row[cellIndex[0]]
            );
            break;
          default:
    ...
    updatedRow["№"] = { value: rowIndex + 1 };
    return updatedRow;
  };

  return data.map((row, index) => createRow(flatColumns, row, index));
};
