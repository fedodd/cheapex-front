...
class Form extends Component {
  ...
  handleSubmit = (event) => {
    ...
    promise.then(
      (result) => {
        ...
        axios
          .post(
            "https://react-app-bc4e6.firebaseio.com/importedSheet.json",
            sendData
          )
          .then((response) => {
          ...
          .then((response) => {
            axios
              ...
              .then((response) => {
                ...
                this.setState({
                  resultsId: resultsIdOld.concat(
                    lastItemName[lastItemName.length - 1]
                  ),
  ...
  render() {
    ...
    if (this.state.importedDataLink !== "") {
      newDataLink = (
        <p>
          . data imported succesfull! link to imported data:{" "}
          {this.state.importedDataLink}
        </p>
      );
    }
